...
async def add_book_manually(
    ...
    # Если файл передан, сохраняем его локально
    if cover_file:
        file_extension = cover_file.filename.split(".")[-1]
        ...
        # Формируем URL (замените localhost на ваш IP для теста на телефоне)
        final_cover = f"/static/{file_name}"

    # Собираем данные обратно в объект для CRUD
    from app.schemas.custom_book import BookManualCreate
    book_data = BookManualCreate(
        title=title, author=author, category_name=category_name,
        pages=pages, year=year, language=language,
        description=description, isbn=isbn, cover_url=final_cover
    )

    review = crud.create_manual_book_review(db, user_id, book_data)
    return {"message": "Book added", "review_id": review.id}
